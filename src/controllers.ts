...
class GlobalControllerClass {
  ...
  async createChatSession(req: Request, res: Response) {
    try {
      ...
      const existingSession =
        await ChatSessionRepository.getExistingSession(user_ids);

      if (existingSession?.length) {
        // If there is an existing session, fetch the existing session along with its participants
        console.log(existingSession);
        const chatSession = await ChatSessionRepository.getChatSession({
          chat_session_id: existingSession[0].dataValues.chat_session_id,
        });
        res.status(201).json({ data: chatSession });
        return;
      }

      const newChatSession = await ChatSessionRepository.createOneChatSession(
        chat_name ?? "",
      );
  ...
  async getChatSessions(req: Request, res: Response) {
    try {
      ...
      const userChatSessions = await ChatSessionRepository.getChatSessions({
      ...
      for (let i = 0; i < userChatSessions?.length; i++) {
        const otherParticipants = await ParticipantRepository.getParticipants(
          userChatSessions[i].dataValues.chat_session_id,
        );
      ...
      res
        .status(500)
        .json({
          message: "something went wrong while fetching your chat sessions",
        });
    }
  }
